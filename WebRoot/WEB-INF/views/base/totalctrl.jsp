...
<title>阀门控制</title>
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/maintenance/dataGridTotalCtrl',
			fit : true,
			striped : true,
			rownumbers : true,
			pagination : true,
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					if(value == '3051'){return '远程阀门控制';}
					if(value == '3052'){return '现场阀门控制';}
				}
			}, {
				width : '140',
				title : '状态',
	...
	function tiFun(){
		parent.$.modalDialog({
			...
			width : 250,
			height : 150,
			href : '${ctx}/maintenance/totalTiPage',
			buttons : [ {
				text : '确定',
				handler : function() {
	...
	function ctrlFun(){
		parent.$.modalDialog({
			...
			width : 250,
			height : 200,
			href : '${ctx}/maintenance/totalCtrlPage',
			buttons : [ {
				text : '确定',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
		...
		<form id="searchForm" method="post">
			<table>
				<tr>
					...
					<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-btn',plain:true" onclick="ctrlFun();">控制阀门</a></td>
				</tr>
			</table>
		</form>
	</div>
